...
angular.module('app.controllers', ['app.config']).controller('AppCtrl', [
  ...
  '$scope', function($scope) {
    ...
    $scope.init = function(opts) {
      ...
      $scope.$watch('page', function(page, oldVal, scope) {
        return $scope.load();
      }, true);
      return $scope.$watch('filter_opts ', function(filter_opts, oldVal, scope) {
        return $scope.load();
      }, true);
    };
...
angular.module('app.datas.controllers', ['app.config']).controller('TestExamplesCtrl', [
  '$scope', '$http', '$routeParams', 'settings', '$location', 'Data', 'Model', 'TestResult', function($scope, $http, $routeParams, settings, $location, Data, Model, Test) {
    ...
    $scope.model.$load({
      ...
      return $scope.$watch('filter_opts', function(filter_opts, oldVal, scope) {
        return $location.search(filter_opts);
      }, true);
    }), (function() {
  ...
  '$http', '$q', 'settings', function($http, $q, settings) {
    ...
    Data = (function() {
      ...
      */
      ...
      Data.$loadAllGroupped = function(opts) {
        var dfd,
          _this = this;
        dfd = $q.defer();
        $http({
...
var createSVG, getPlotData, updateCurves, zip;
  ...
  function() {
    return {
      ...
      link: function(scope, element, attrs) {
        ...
        return scope.$watch('curvesDict', updateCurves);
      }
    };
  }
]);

createSVG = function(scope, element, width, height) {
  if (width == null) {
    width = 400;
  }
  if (height == null) {
  ...
  if (!(scope.svg != null)) {
    return scope.svg = d3.select(element[0]).append("svg").attr("width", width).attr("height", height);
  }
};

updateCurves = function(curvesDict, oldVal, scope) {
  var chart;
  if (!curvesDict) {
    return;
  }
  chart = nv.models.lineChart();
  chart.xAxis.orient('bottom').axisLabel(scope.xLabel).tickFormat(d3.format(',r'));
  chart.yAxis.orient('left').axisLabel(scope.yLabel).tickFormat(d3.format(',.f'));
  scope.svg.datum(getPlotData(curvesDict, scope.showLine)).transition().duration(500).call(chart);
...
getPlotData = function(curvesDict, addLine) {
  var data, i, line_data, plotData, plotName, plots_data, step, values;
  plots_data = [];
  for (plotName in curvesDict) {
    plotData = curvesDict[plotName];
    if ((plotData != null) && (plotData[0] != null) && (plotData[1] != null)) {
...
angular.module('app.importhandlers.controllers', ['app.config']).controller('ImportHandlerListCtrl', [
  '$scope', '$http', '$dialog', 'settings', 'ImportHandler', function($scope, $http, $dialog, settings, ImportHandler) {
    return ImportHandler.$loadAll({
  ...
  '$scope', '$http', '$location', '$routeParams', '$dialog', 'settings', 'ImportHandler', function($scope, $http, $location, $routeParams, $dialog, settings, ImportHandler) {
    var err;
    if (!$scope.model) {
      if (!$routeParams.name) {
  ...
  '$scope', '$http', '$location', 'settings', 'ImportHandler', function($scope, $http, $location, settings, ImportHandler) {
    ...
    $scope.add = function() {
      ...
      return $scope.handler.$save().then((function() {
    ...
    return $scope.setDataFile = function(element) {
      return $scope.$apply(function($scope) {
        ...
        reader.onload = function(e) {
          var str;
          str = e.target.result;
          return $scope.handler.data = str;
        };
        return reader.readAsText($scope.data);
      });
    };
  }
]);
  ...
  '$http', '$q', 'settings', function($http, $q, settings) {
    ...
    */
    ...
    ImportHandler = (function() {
      ...
      ImportHandler.prototype.id = null;

      ImportHandler.prototype.created_on = null;

      ImportHandler.prototype.updated_on = null;

      ImportHandler.prototype.name = null;

      ImportHandler.prototype.type = null;

      ImportHandler.prototype.data = null;

      /* API methods
      */


      ImportHandler.prototype.objectUrl = function() {
        return '/import_handlers/' + this.name;
      };

      ImportHandler.prototype.isNew = function() {
        if (this.id === null) {
          return true;
        } else {
          return false;
      ...
      ImportHandler.prototype.$load = function(opts) {
        ...
        return $http({
          method: 'GET',
          url: "" + settings.apiUrl + "import/handler/" + this.name,
          headers: {
            'X-Requested-With': null
          },
          params: _.extend({}, opts)
          ...
          _this.loaded = true;
          _this.loadFromJSON(resp.data['import_handler']);
          return resp;
        }), (function(resp) {
          return resp;
        }));
      ...
      ImportHandler.prototype.$save = function(opts) {
        var fd,
          _this = this;
        if (opts == null) {
          opts = {};
        ...
        return $http({
          ...
          return _this.loadFromJSON(resp.data['import_handler']);
        });
      };

      ImportHandler.$loadAll = function(opts) {
        var dfd,
          _this = this;
        dfd = $q.defer();
        $http({
          ...
          return dfd.resolve({
            ...
            objects: (function() {
              var _i, _len, _ref, _results;
              _ref = resp.data.import_handlers;
              _results = [];
              for (_i = 0, _len = _ref.length; _i < _len; _i++) {
                obj = _ref[_i];
...
LOCAL_SETTINGS = {
  apiUrl: 'http://127.0.0.1:5000/cloudml/'
};
...
angular.module('app.models.controllers', ['app.config']).controller('ModelListCtrl', [
  '$scope', '$http', '$dialog', 'settings', 'Model', function($scope, $http, $dialog, settings, Model) {
    return Model.$loadAll({
      ...
      return $scope.err = "Error while saving: server responded with " + ("" + opts.status + " ") + ("(" + (opts.data.response.error.message || "no message") + "). ") + "Make sure you filled the form correctly. " + "Please contact support if the error will not go away.";
    }));
  }
]).controller('AddModelCtl', [
  ...
  '$scope', '$http', '$location', '$routeParams', '$dialog', 'settings', 'Model', 'TestResult', function($scope, $http, $location, $routeParams, $dialog, settings, Model, Test) {
    ...
    $scope.$watch('action', function(action) {
      ...
      switch (action[0]) {
        ...
        case "weights":
          return $scope.goWeights(true, true);
        case "test":
          return $scope.goTests();
        case "import_handlers":
          if (action[1] === 'train') {
            return $scope.go('train_importhandler,status,id');
          } else {
            return $scope.go('importhandler,status,id');
          }
          break;
        default:
          return $scope.go('status,created_on,target_variable,error');
      }
    });
    if (!$scope.model) {
      if (!$routeParams.name) {
    ...
    $scope.goWeights = function(morePositive, moreNegative) {
      return $scope.model.$loadWeights({
        show: 'status,name',
        ppage: $scope.ppage,
        npage: $scope.npage
      }).then((function(resp) {
        if (morePositive) {
          $scope.positive.push.apply($scope.positive, $scope.model.positive_weights);
        }
        if (moreNegative) {
          return $scope.negative.push.apply($scope.negative, $scope.model.negative_weights);
        }
      }), (function() {
        return $scope.err = data;
      }));
    };
    $scope.morePositiveWeights = function() {
      $scope.ppage += 1;
      return $scope.goWeights(true, false);
    };
    $scope.moreNegativeWeights = function() {
      $scope.npage += 1;
      return $scope.goWeights(false, true);
    };
    $scope.goTests = function() {
      return Test.$loadTests($scope.model.name, {
        show: 'name,created_on,status,parameters,accuracy,examples_count'
      }).then((function(opts) {
        return $scope.tests = opts.objects;
      }), (function(opts) {
        return $scope.err = "Error while loading tests: server responded with " + ("" + opts.status + " ") + ("(" + (opts.data.response.error.message || "no message") + "). ");
      }));
    };
    ...
    Model = (function() {
      ...
      */
      ...
      Model.prototype.loadFromJSON = function(origData) {
        ...
        _.extend(this, data);
        if (origData != null) {
          this.created_on = String(origData['created_on']);
      ...
      Model.prototype.$loadWeights = function(opts) {
        var _this = this;
        if (this.name === null) {
          throw new Error("Can't load model without name");
        }
        return $http({
          method: 'GET',
          url: settings.apiUrl + ("model/" + this.name + "/weights"),
          headers: {
            'X-Requested-With': null
          },
          params: _.extend({}, opts)
...
angular.module('app.reports.controllers', ['app.config']).controller('CompareModelsFormCtl', [
  '$scope', '$http', '$location', '$routeParams', 'settings', 'Model', 'TestResult', 'Data', 'CompareReport', function($scope, $http, $location, $routeParams, settings, Model, Test, Data, CompareReport) {
    ...
    $scope.loadTestsList = function(model) {
      return Test.$loadTests(model.name, {
        status: "Completed"
      }).then((function(opts) {
      ...
      */
      ...
      CompareReport.prototype.$getReportData = function() {
        ...
        return $http({
          ...
          var auc, example, examples, examples_data, key, num, test, value, _i, _len, _ref;
          _this.generated = true;
          _this.tests = [];
          _this.rocCurves = {};
          _this.precisionRecallCurves = {};
          _ref = resp.data;
          for (key in _ref) {
            value = _ref[key];
            if (key.indexOf('test') === 0) {
              ...
              _this.rocCurves[test.fullName() + " (AUC = " + auc + ")"] = test.metrics.roc_curve;
              _this.precisionRecallCurves[test.fullName()] = test.metrics.precision_recall_curve.reverse();
            }
...
angular.module('app.testresults.controllers', ['app.config']).controller('TestDialogController', [
  ...
  '$scope', '$http', '$routeParams', 'settings', 'TestResult', '$location', function($scope, $http, $routeParams, settings, Test, $location) {
    ...
    $scope.$watch('action', function(action) {
      ...
      switch (action[0]) {
        case "curves":
          return $scope.goMetrics();
        case "matrix":
          return $scope.go('status,metrics.confusion_matrix');
        default:
          return $scope.go('name,status,classes_set,created_on,accuracy,\
parameters,error,examples_count');
    ...
    return $scope.goMetrics = function(fields, callback) {
      var _this = this;
      return $scope.go('status,metrics.roc_curve,\
metrics.precision_recall_curve,metrics.roc_auc', function() {
        var pr;
        $scope.rocCurve = {
          'ROC curve': $scope.test.metrics.roc_curve
        };
        pr = $scope.test.metrics.precision_recall_curve;
        return $scope.prCurve = {
          'Precision-Recall curve': [pr[1], pr[0]]
        };
      });
    };
  }
    ...
    */
    ...
    TestResult = (function() {
      ...
      function TestResult(opts) {
        ...
        this.examplesUrl = __bind(this.examplesUrl, this);

        this.objectUrl = __bind(this.objectUrl, this);
        this.loadFromJSON(opts);
      }

      TestResult.prototype._id = null;
      ...
      */
      ...
      TestResult.prototype.objectUrl = function() {
        return '/models/' + (this.model_name || this.model.name) + "/tests/" + this.name;
      };

      TestResult.prototype.examplesUrl = function() {
        var model;
        model = this.model_name || this.model.name;
        return "/models/" + model + "/tests/" + this.name + "/examples";
      };

      TestResult.prototype.fullName = function() {
        if ((this.model != null) || this.model_name) {
          return (this.model_name || this.model.name) + " / " + this.name;
        }
        return this.name;
      };
